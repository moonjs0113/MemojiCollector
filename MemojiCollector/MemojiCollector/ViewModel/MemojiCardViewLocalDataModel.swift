//
//  MemojiCardViewLocalDataModel.swift
//  MemojiCollector
//
...
class MemojiCardViewLocalDataModel: ObservableObject {
    ...
    func updateImageData(imageData: Data) {
        ...
        var cardList = JsonManagerClass.shared.jsonDecoder(decodingData: cardListData)
        if let memojiCard = self.memojiCard {
            cardList.indices.filter{
                ...
                cardListData = JsonManagerClass.shared.jsonEncoder(ecodingData: cardList)
            }
        }
    }
